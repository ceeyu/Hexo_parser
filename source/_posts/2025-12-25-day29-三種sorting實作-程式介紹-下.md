---
title: DAY29 三種sorting實作-程式介紹(下)
date: 2022-09-30 29:00:00
tags: []

source: https://ithelp.ithome.com.tw/articles/10294889
series: C++ 基礎教學
categories:
  - C++ 基礎教學
---
...
// sorting類別包含三個sort
{
     public:
          ...
          // tree不會用到arr[0]方便對照
          void printAll(int n);//printAll是從0~n+1
...
程式中我們先將陣列分為前後段，並在for迴圈內寫出兩個條件段，進行兩段數列自前項到後項index得值一一比較並排序放入result[]陣列中。而下一個if判斷條件copy data的部分(因為陣列可能不剛好為偶數)。
...
在mergePass()的部分是為了要為陣列分出段落而定的函式。
...
mergeSort()就是結合兩者而定的函式。

### 第二部分:成果展示

![https://ithelp.ithome.com.tw/upload/images/20220930/20151593OkP2JQcQzR.jpg](https://ithelp.ithome.com.tw/upload/images/20220930/20151593OkP2JQcQzR.jpg)
